...
for icond = 1:6
    for isession = 1:5
        ...
        ti = cat(1,ti,[log2(s(isession).session.cond(icond).meanDRon), log2(s(isession).session.cond(icond+6).meanDRon)]);
        ci = cat(1,ci,icond);
    end
end
xlim(log2([1 10]))
...
for icond = 1:6
    for isession = 1:5
        plot(log2(s(isession).session.cond(icond).meanDRoff), log2(s(isession).session.cond(icond+6).meanDRoff),...
            'o', 'MarkerEdgeColor','none', 'MarkerFaceColor',speedcols(icond,:))
        tioff=cat(1,tioff, [log2(s(isession).session.cond(icond).meanDRoff), log2(s(isession).session.cond(icond+6).meanDRoff)]);
    end
end
xlim(log2([1 10]))
ylim(log2([1 10]))
